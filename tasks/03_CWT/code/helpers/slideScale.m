function [position, ConfTimeStamp, RT, answer] = slideScale(keys, screenPointer, question, rect, anchors, varargin)
...
% responseKeys  = [keys.One keys.Two keys.Three];
responseKeys.One = keys.One;
responseKeys.Two = keys.Two;
responseKeys.Three = keys.Three;
GetMouseIndices;
drawImage     = 0;
...
while(i<=length(varargin))
    switch lower(varargin{i})
        ...
        case 'displayposition'
            ...
            i             = i + 1;
        case 'categorical_slider'
            i                       = i + 1;
            categorical_slider      = varargin{i};
            i                       = i + 1;
        case 'trigger_rect'
            i             = i + 1;
            trigger_rect  = varargin{i};
            i             = i + 1;
        case 'colour_rect'
            i             = i + 1;
...
if categorical_slider == 1
    stepSize = scalaLength_pix/10;
end
while answer == 0
